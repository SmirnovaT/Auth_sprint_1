...
async def test_get_all_auth_histories_wo_access_401(
        client_session, delete_row_from_table, make_post_request,
):
    client_session.cookie_jar.clear()

    _, registration_response = await make_post_request(USER_URL + "/signup", registration_data)
    ...
    async with client_session.get(
        ...
        await delete_row_from_table(
            "users", registration_response["id"]
        )


async def test_all_auth_histories_success_200(
    put_data, access_token_admin, client_session,
    delete_row_from_table, make_post_request,
):
    ...
    async with client_session.get(f"{AUTH_HISTORY_URL}/{registration_response["id"]}") as raw_response:
        response = await raw_response.json()
