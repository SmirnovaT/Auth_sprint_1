...
async def test_refresh_token_401(make_post_request):
    ...
    assert response["detail"] == 'Пользователь не авторизован, нет рефреш токена в cookies'


async def test_refresh_token_200(refresh_token_unknown_role, client_session):
    client_session.cookie_jar.update_cookies({"refresh_token": refresh_token_unknown_role})
...
async def test_user_creating_w_already_existing_name(
    ...
    status, response_error = await make_post_request(USER_URL + "/signup", registration_data)
    assert status == 400
